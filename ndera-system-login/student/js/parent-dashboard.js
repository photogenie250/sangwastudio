...
const YEAR_KEY = 'sdms_parent_year_of_birth';
...
const studentAvatarInitialsEl = document.getElementById('student-avatar-initials');
...
function renderStudent(student, underCounseling) {
  ...
  studentAvatarInitialsEl.textContent = initials || '🙂';
  topbarAvatarEl.textContent = initials || '🙂';
  // Photo (if any) is fetched and rendered separately by
  // photo-upload.js — see supabase/migrations/0008_student_photos.sql
  // — so this function stays focused on the read-only lookup data.
  document.dispatchEvent(new CustomEvent('sdms:profile-loaded'));
  ...
  // Prefill the "write to DOD" link with the student's name/class/
  // number so the parent doesn't have to retype what we already
  // know — and so the resulting request row is linked to this exact
  // student (via student_number) and shows up in "Uruhushya rwasabwe"
  // below, instead of being an unlinked free-text submission.
...
// ------------------------------------------------------------
// Uruhushya rwasabwe — this student's own permission requests
// and DOD's decision on each, from parent_portal_permission_requests
// (see supabase/migrations/0011_permission_request_decisions.sql).
// Card stays hidden if the parent has never submitted a request
// with this student's SDMS number attached.
// ------------------------------------------------------------
const PERMISSION_STATUS_LABELS = {
  pending: 'Birikugenzurwa',
  approved: 'Byemejwe',
  declined: 'Byanzwe',
};
...
function renderPermissionRequests(requests) {
  ...
  permissionList.innerHTML = requests.map((r) => `
    <div class="parent-counseling-row">
      <span class="session-status-badge session-status-badge--${escapeHtml(r.status)}">${escapeHtml(PERMISSION_STATUS_LABELS[r.status] || r.status)}</span>
      <div>
        <p class="parent-counseling-row__reason">${escapeHtml(r.reason)}</p>
        <p class="parent-counseling-row__date">${formatDate(r.created_at)}${r.staff_note ? ` — ${escapeHtml(r.staff_note)}` : ''}</p>
      </div>
    </div>
  `).join('');
}
...
async function init() {
  ...
  const yearOfBirth = sessionStorage.getItem(YEAR_KEY);
  ...
  const { data, error } = await supabase.rpc('parent_portal_lookup', {
    ...
    p_year_of_birth: parseInt(yearOfBirth, 10),
  });
  ...
  // Permission requests — this student's own submissions and DOD's
  // decision on each. Fetched alongside likes/announcements, on its
  // own — if it fails, the card just stays hidden rather than
  // blocking the rest of the dashboard.
  supabase.rpc('parent_portal_permission_requests', {
    p_student_number: code,
    p_year_of_birth: parseInt(yearOfBirth, 10),
  }).then(({ data: permData, error: permError }) => {
    if (permError) {
      console.error('parent_portal_permission_requests failed:', permError.message);
      permissionCard.hidden = true;
      return;
    }
    renderPermissionRequests((permData && permData.found) ? permData.requests : []);
  });
