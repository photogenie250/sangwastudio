...
## Marks protection for approved requests (0019)

Once a permission request is **approved**, the student it belongs to can
no longer have an incident recorded against them for that same date —
`0019_dod_authorization_marks_protection.sql` adds a database trigger on
`incidents` that refuses the insert outright and the staff app shows
"authorised by DoD" instead of letting marks be removed. This is enforced
in the database itself (not just in the student-profile screen), so it
holds for every client that can ever insert into `incidents`, including
the Android app.
