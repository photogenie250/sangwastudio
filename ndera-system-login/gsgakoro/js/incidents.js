...
async function loadOffenses() {
  ...
  offenseSelect.innerHTML = '<option value="" disabled selected>Select an offense…</option>' +
    Object.entries(grouped).map(([category, offenses]) => `
      <optgroup label="${escapeHtml(category)}">
        ${offenses.map((o) => `<option value="${o.id}">${escapeHtml(o.title)} (−${o.deduction})</option>`).join('')}
      </optgroup>
    `).join('');

  offenseReferenceList.innerHTML = Object.entries(grouped).map(([category, offenses]) => `
...
form.addEventListener('submit', async (e) => {
  ...
  // on the insert above — that WhatsApp alert doesn't depend on the
