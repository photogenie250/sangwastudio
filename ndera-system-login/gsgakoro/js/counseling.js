...
//   id, student_id, reason, scheduled_date, status, notes,
//   assigned_teacher_id, created_by, created_at
// with status in ('scheduled', 'completed', 'cancelled').
//
// Case assignment (assigned_teacher_id): counseling was previously
// always handled by whoever scheduled it (administrator/head_teacher
// — in practice, the Discipline Office alone). A case can now be
// assigned to a specific staff member instead. Only administrator/
// head_teacher can set/change the assignment (see the "Assign to"
// field, canSchedule-gated). The assigned staff member — including
// a plain 'teacher' who otherwise has no counseling access — can
// then open their case and write the notes/status themselves; those
// notes feed the exact same printed "Counseling report" the
// Discipline Office/Headteacher relies on. DB-side, this is enforced
// by the counseling_sessions_update_assigned_teacher RLS policy plus
// the enforce_counseling_assigned_update_scope trigger (status/notes
// only for a non admin/head_teacher updater) — see migration 0020.
// ============================================================
...
const sessionStudentField = document.getElementById('session-student-field');
const sessionStudentSelect = document.getElementById('session-student-select');
...
const sessionAssigneeField = document.getElementById('session-assignee-field');
const sessionAssigneeSelect = document.getElementById('session-assignee-select');
const sessionAssigneeReadonly = document.getElementById('session-assignee-readonly');
const sessionAssigneeReadonlyText = document.getElementById('session-assignee-readonly-text');
...
async function loadEligibleStudents() {
  ...
  // Kept separately (unfiltered) so the "Add student to counseling"
  // manual picker can offer every active student, not just the ones
  // who currently meet the eligibility criteria below.
...
function renderEligible(students) {
  ...
  const canSchedule = currentRole === 'administrator' || currentRole === 'head_teacher';
...
async function loadSessions() {
  ...
  const { data, error } = await supabase
    ...
    .select('id, student_id, reason, scheduled_date, status, notes, assigned_teacher_id, students ( first_name, last_name, student_number ), assigned_teacher:profiles!counseling_sessions_assigned_teacher_id_fkey ( first_name, last_name )')
    .order('scheduled_date', { ascending: false });

  if (error) {
    console.error('Failed to load counseling sessions:', error);
    sessionsBody.innerHTML = `<tr><td colspan="6" class="data-table__empty">Could not load counseling sessions.</td></tr>`;
    return;
  }

  // A plain teacher (not admin/head_teacher/discipline_teacher) only
  // has counseling access at all because a case may be assigned to
  // them — scope the log down to their own assignment(s) so they
  // aren't reading every student's counseling record.
  sessionsCache = currentRole === 'teacher'
    ? (data ?? []).filter((s) => s.assigned_teacher_id === currentUserId)
    : (data ?? []);
...
function renderSessions(sessions) {
  ...
  const emptyMessage = currentRole === 'teacher'
    ? 'No counseling cases are assigned to you.'
    : 'No counseling sessions logged yet.';

  if (sessions.length === 0) {
    sessionsBody.innerHTML = `<tr><td colspan="6" class="data-table__empty">${emptyMessage}</td></tr>`;
  ...
  sessionsBody.innerHTML = sessions.map((s) => {
    const isOwnAssignment = s.assigned_teacher_id === currentUserId;
    const assigneeLabel = s.assigned_teacher
      ? `${s.assigned_teacher.first_name ?? ''} ${s.assigned_teacher.last_name ?? ''}`.trim()
      : '—';
    return `
    <tr data-id="${s.id}">
      ...
      <td>
        <div class="row-actions">
          ...
          ${!isAdmin && isOwnAssignment ? '<button class="row-action-btn" data-action="write-notes">Write notes</button>' : ''}
        </div>
      </td>
    </tr>
  `;
  ...
  sessionsBody.querySelectorAll('[data-action="write-notes"]').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      const id = e.target.closest('tr').dataset.id;
      const record = sessionsCache.find((s) => s.id === id);
      if (record) startWriteNotes(record);
    });
  });
...
// Populates the manual "Student" picker with every active student
// (not just eligible ones), labeled the same way as the eligible
// table so it's easy to find someone by name, SDMS number or class.
function populateStudentSelect() {
  const current = sessionStudentSelect.value;
  sessionStudentSelect.innerHTML =
    `<option value="">Select a student…</option>` +
    allStudentsCache.map((s) => `
      <option value="${s.id}">${escapeHtml(s.first_name)} ${escapeHtml(s.last_name)} · ${escapeHtml(s.student_number)} · ${escapeHtml(s.classes?.class_name ?? 'Unassigned')}</option>
    `).join('');
  if (current) sessionStudentSelect.value = current;
}

sessionStudentSelect.addEventListener('change', () => {
  sessionStudentIdInput.value = sessionStudentSelect.value;
});

// ------------------------------------------------------------
// Assignment — lets an administrator/head_teacher hand a case to
// a specific staff member instead of it defaulting to whoever is
// scheduling it (in practice, the Discipline Office every time).
// ------------------------------------------------------------
...
function populateAssigneeSelect() {
  const current = sessionAssigneeSelect.value;
  sessionAssigneeSelect.innerHTML =
    `<option value="">Unassigned (Discipline Office handles it)</option>` +
    assignableStaffCache.map((p) => `
      <option value="${p.id}">${escapeHtml(`${p.first_name ?? ''} ${p.last_name ?? ''}`.trim())} · ${escapeHtml(p.role.replace('_', ' '))}</option>
    `).join('');
  if (current) sessionAssigneeSelect.value = current;
}

// Toggles the form between full edit (administrator/head_teacher)
// and the restricted "write notes" mode a case's assigned staff
// member gets: student, reason, scheduled date and assignment are
// shown read-only, and only status + notes can be changed — matching
// what the enforce_counseling_assigned_update_scope DB trigger
// actually allows them to save.
function setFormRestricted(restricted) {
  sessionReasonInput.disabled = restricted;
  sessionDateInput.disabled = restricted;
  sessionAssigneeField.hidden = restricted;
  sessionAssigneeReadonly.hidden = !restricted;
}
...
function openSessionForm(student) {
  ...
  sessionStudentField.hidden = true;
  sessionFormStudent.hidden = false;
  ...
  populateAssigneeSelect();
  sessionAssigneeSelect.value = '';
  sessionFormPanel.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}

// Manual add — lets staff schedule counseling for any active
// student, not only the ones auto-flagged by the marks/incidents
// criteria (e.g. a student a teacher is proactively worried about,
// or a family/personal-situation referral that has nothing to do
// with marks or incidents).
function openManualAddForm() {
  ...
  counselingLayout.classList.add('form-open');
  setFormRestricted(false);
  sessionFormTitle.textContent = 'Add student to counseling';
  sessionFormStudent.hidden = true;
  sessionStudentField.hidden = false;
...
function startEditSession(record) {
  ...
  populateAssigneeSelect();
  sessionAssigneeSelect.value = record.assigned_teacher_id ?? '';
  sessionSubmitBtn.textContent = 'Save changes';
  sessionFormError.hidden = true;
}

// Opened by a case's assigned staff member from "Write notes" — same
// form, but scoped to what enforce_counseling_assigned_update_scope
// actually lets them save: status and notes. Everything else about
// the case (student, reason, scheduled date, who it's assigned to)
// is shown for context only.
function startWriteNotes(record) {
  sessionFormPanel.hidden = false;
  counselingLayout.classList.add('form-open');
  setFormRestricted(true);
  sessionStudentField.hidden = true;
  sessionFormStudent.hidden = false;
  sessionFormTitle.textContent = 'Write counseling notes';
  sessionFormStudent.textContent =
    `${record.students?.first_name ?? ''} ${record.students?.last_name ?? ''} · ${record.students?.student_number ?? ''}`.trim();
  sessionIdInput.value = record.id;
  sessionStudentIdInput.value = record.student_id;
  ...
  sessionStatusSelect.value = record.status ?? 'scheduled';
  sessionNotesInput.value = record.notes ?? '';
  sessionAssigneeReadonlyText.textContent = 'You';
  sessionSubmitBtn.textContent = 'Save notes';
  sessionFormError.hidden = true;
  sessionFormPanel.scrollIntoView({ behavior: 'smooth', block: 'nearest' });
}
...
addStudentBtn.addEventListener('click', openManualAddForm);
...
sessionForm.addEventListener('submit', async (e) => {
  ...
  const canAssign = currentRole === 'administrator' || currentRole === 'head_teacher';
  const payload = {
  ...
  // Only an administrator/head_teacher can set or change the
  // assignment — a case's assigned staff member saving their own
  // notes leaves this field out entirely, so it stays untouched
  // (the DB trigger would reject them changing it anyway).
  if (canAssign) {
    payload.assigned_teacher_id = sessionAssigneeSelect.value || null;
  }

  if (!payload.student_id) {
    sessionFormError.textContent = 'Choose a student before saving.';
...
function buildReportHtml(session) {
  ...
  const assignedTo = session.assigned_teacher
    ? `${session.assigned_teacher.first_name ?? ''} ${session.assigned_teacher.last_name ?? ''}`.trim()
    : 'Discipline Office';

  return `
    <div class="report-paper">
      <div class="report-letterhead">
        <img src="../assets/img/ndera-logo.svg" alt="NDERA SYSTEM" class="report-logo">
        <div>
      ...
      <table class="report-details">
        <tbody>
          ...
          <tr><th>Student SDMS</th><td>${escapeHtml(studentNumber)}</td></tr>
          <tr><th>Reason</th><td>${escapeHtml(session.reason || '—')}</td></tr>
          <tr><th>Counselled by</th><td>${escapeHtml(assignedTo)}</td></tr>
...
(async function init() {
  ...
  const canSchedule = currentRole === 'administrator' || currentRole === 'head_teacher';
  addStudentBtn.hidden = !canSchedule;

  // A plain teacher only has counseling access because a case may
  // be assigned to them — they don't need (and can't act on) the
  // eligible-students list or the "print all" bulk action, which
  // pulls in every student's report.
  if (currentRole === 'teacher') {
    const eligiblePanel = eligibleBody.closest('.panel');
    if (eligiblePanel) eligiblePanel.hidden = true;
    printAllReportsBtn.hidden = true;
  }

  const tasks = [loadSessions()];
  if (currentRole !== 'teacher') tasks.push(loadEligibleStudents());
  if (canSchedule) tasks.push(loadAssignableStaff());
