...
async function loadRequests() {
  ...
  if (error) {
    ...
    requestsCards.innerHTML = `<li class="request-cards__empty">Could not load requests.</li>`;
...
function renderRequests() {
  ...
  if (filtered.length === 0) {
    ...
    requestsCards.innerHTML = `<li class="request-cards__empty">No requests match.</li>`;
    return;
  }

  requestsBody.innerHTML = filtered.map(renderRow).join('');
  requestsCards.innerHTML = filtered.map(renderCard).join('');

  [requestsBody, requestsCards].forEach((container) => {
...
// Shared by both the desktop table row and the mobile card —
// pending requests get Approve/Decline buttons, decided ones show
// the note staff left (which is also what the parent sees).
...
function renderRow(r) {
  return `
    <tr>
      ...
      <td class="permission-actions-cell">${renderActions(r)}</td>
    </tr>
  `;
}

// Mobile card — same data as the table row, laid out to read
// top-to-bottom on a narrow screen instead of scrolling sideways.
function renderCard(r) {
  return `
    <li class="request-card">
      <div class="request-card__top">
        <p class="request-card__student">${escapeHtml(r.student_name)}</p>
        <span class="status-badge status-badge--${escapeHtml(r.status)}">${escapeHtml(r.status)}</span>
      </div>
      <p class="request-card__meta">${escapeHtml(r.student_class ?? '—')} &middot; ${formatDateTime(r.created_at)}</p>

      <p class="request-card__reason">${escapeHtml(r.reason)}</p>

      <div class="request-card__requester">
        <p class="request-card__requester-name">${escapeHtml(r.requester_name)}</p>
        <a class="request-card__phone" href="tel:${escapeHtml(r.requester_phone ?? '')}">${escapeHtml(r.requester_phone)}</a>
      </div>

      <div class="request-card__actions">${renderActions(r)}</div>
    </li>
  `;
}
