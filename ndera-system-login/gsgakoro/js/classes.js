...
async function loadClasses() {
  const { data, error } = await supabase
    .from('classes')
    .select('id, class_name, level, academic_year, teacher_id, profiles ( first_name, last_name )')
    .order('academic_year', { ascending: false })
    .order('class_name');

  if (error || !data) {
    classesBody.innerHTML = `<tr><td colspan="5" class="data-table__empty">Could not load classes.</td></tr>`;
    return;
  }
...
function renderClasses(classes) {
  if (classes.length === 0) {
    classesBody.innerHTML = `<tr><td colspan="5" class="data-table__empty">No classes yet.</td></tr>`;
    return;
  }
