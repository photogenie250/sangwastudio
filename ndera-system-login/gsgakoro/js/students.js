...
import { getCurrentTerm } from './term.js';
...
// Profile photo (captured on the parent/student portal)
...
// Marks-removal confirmation dialog
const marksConfirm = document.getElementById('marks-confirm');
const confirmPhotoImg = document.getElementById('confirm-photo-img');
const confirmPhotoInitials = document.getElementById('confirm-photo-initials');
const confirmNameEl = document.getElementById('marks-confirm-title');
const confirmMetaEl = document.getElementById('confirm-meta');
const confirmNoPhotoEl = document.getElementById('confirm-no-photo');
const confirmOffenseEl = document.getElementById('confirm-offense');
const confirmDateEl = document.getElementById('confirm-date');
const confirmMarksEl = document.getElementById('confirm-marks');
...
// Identity of the student whose profile is open, kept so the
// marks-removal dialog can show their photo/number/class without a
// second round trip at the moment of confirming.
...
// uburenganzira"), or null if the teacher cancelled.
//
// This used to be a window.prompt(), which could only put the
// student's name in front of the teacher as a line of text. Marks
// are irreversible from here, so the dialog now leads with the
// student's own photo — the one taken on their phone in the
// parent/student portal — next to their number and class. A teacher
// who opened the wrong row, or picked the wrong one of two similar
// names, sees a face that doesn't match before anything is deducted.
// The typed-name authorisation is unchanged.
function confirmMarksRemoval({ fullName, studentNumber, className, photoUrl, offenseTitle, deduction, incidentDate }) {
  return new Promise((resolve) => {
    setStudentPhoto(confirmPhotoImg, confirmPhotoInitials, photoUrl, fullName);
    confirmNameEl.textContent = fullName || 'Unknown student';
    confirmMetaEl.textContent = `${studentNumber ?? '—'} · ${className ?? 'Unassigned'}`;
    confirmNoPhotoEl.hidden = Boolean(photoUrl);
    confirmOffenseEl.textContent = offenseTitle || '—';
    confirmDateEl.textContent = incidentDate ? formatDate(incidentDate) : '—';
    confirmMarksEl.textContent = deduction === '' || deduction == null ? '—' : `−${deduction}`;

    confirmNameInput.value = '';
    marksConfirm.hidden = false;
    confirmNameInput.focus();

    function close(result) {
      marksConfirm.hidden = true;
      confirmCancelBtn.removeEventListener('click', onCancel);
      confirmAcceptBtn.removeEventListener('click', onAccept);
      marksConfirm.removeEventListener('click', onBackdrop);
      document.removeEventListener('keydown', onKeydown);
      confirmNameInput.removeEventListener('keydown', onInputKeydown);
      resolve(result);
    }

    // Cancelling, dismissing with Escape and clicking the backdrop all
    // mean "I didn't mean to do this" — all resolve to null so no
    // error message is shown, matching the old prompt's behaviour.
    function onCancel() { close(null); }
    function onAccept() {
      close(normalizeName(confirmNameInput.value) === normalizeName(currentTeacherName));
    }
    function onBackdrop(e) { if (e.target === marksConfirm) close(null); }
    function onKeydown(e) { if (e.key === 'Escape') close(null); }
    function onInputKeydown(e) { if (e.key === 'Enter') { e.preventDefault(); onAccept(); } }

    confirmCancelBtn.addEventListener('click', onCancel);
    confirmAcceptBtn.addEventListener('click', onAccept);
    marksConfirm.addEventListener('click', onBackdrop);
    document.addEventListener('keydown', onKeydown);
    confirmNameInput.addEventListener('keydown', onInputKeydown);
  });
}
...
async function loadStudents() {
  ...
  let query = supabase
    .from('students')
    .select('id, student_number, first_name, last_name, gender, year_of_birth, parent_phone, parent_email, class_id, current_marks, status, photo_url, classes ( class_name )', { count: 'exact' });
    ...
    <tr data-id="${s.id}">
      <td>
        <div class="student-cell">
          ${studentPhotoMarkup(s.photo_url, `${s.first_name} ${s.last_name}`)}
          <div class="student-cell__text">
            ${escapeHtml(s.first_name)} ${escapeHtml(s.last_name)}${counselingAttentionIds.has(s.id) ? ' <span class="attention-badge">Under counseling</span>' : ''}${likeCountsById[s.id] ? ` <span class="like-badge">👍 ${likeCountsById[s.id]}</span>` : ''}<br><span class="data-table__meta" style="font-family: var(--font-mono); font-size: 11px; color: var(--charcoal-soft);">${escapeHtml(s.student_number)}</span>
          </div>
        </div>
      </td>
...
async function openProfile(studentId) {
  ...
  setStudentPhoto(profilePhotoImg, profilePhotoInitials, null, '');
  ...
  // The photo isn't part of get_student_report (that function is
  // report-focused and shared with the reports page), so it's read
  // directly — one extra column on one row.
  ...
  setStudentPhoto(profilePhotoImg, profilePhotoInitials, photoRow?.photo_url, r.full_name);
...
profileIncidentForm?.addEventListener('submit', async (e) => {
  ...
  // Re-read straight from the database rather than trusting what's on
  // screen: if this tab has been open a while (or a photo was taken on
  // the student's phone since), the confirmation below must show what
  // the record actually holds right now.
  const { data: studentRow } = await supabase
    .from('students')
    .select('first_name, last_name, parent_email, student_number, photo_url, classes ( class_name )')
    .eq('id', currentProfileStudentId)
    .single();

  if (studentRow) {
    currentProfileIdentity = {
      fullName: `${studentRow.first_name ?? ''} ${studentRow.last_name ?? ''}`.trim(),
      studentNumber: studentRow.student_number,
      className: studentRow.classes?.class_name ?? null,
      photoUrl: studentRow.photo_url ?? null,
    };
    setStudentPhoto(profilePhotoImg, profilePhotoInitials, studentRow.photo_url, currentProfileIdentity.fullName);
  }
  ...
  // A student with an APPROVED permission request for this exact date
  // keeps their marks — no incident (and therefore no deduction) may
  // be recorded against them for that date. Checked here for an
  // immediate, friendly message; the database also refuses the insert
  // below regardless of this check (see 0019_dod_authorization_marks_
  // protection.sql), so the rule holds everywhere in the system, not
  // just in this form.
  const { data: isAuthorized, error: authCheckError } = await supabase.rpc(
    'permission_authorizes_student',
    { p_student_id: currentProfileStudentId, p_date: incidentDate }
  );

  if (authCheckError) {
    console.error('Failed to check DoD authorization:', authCheckError);
  }

  if (isAuthorized) {
    showProfileIncidentError(
      `${studentFullNameForConfirm || 'This student'} was authorised by DoD for ${formatDate(incidentDate)} — marks were not removed.`
    );
    return;
  }

  const confirmation = await confirmMarksRemoval({
    fullName: studentFullNameForConfirm,
    studentNumber: currentProfileIdentity?.studentNumber,
    className: currentProfileIdentity?.className,
    photoUrl: currentProfileIdentity?.photoUrl,
    offenseTitle: offenseForConfirm?.title,
    deduction: offenseForConfirm?.deduction ?? '',
    incidentDate,
  });
  ...
  if (insertError) {
    ...
    // The database itself refuses this insert (trigger in
    // 0019_dod_authorization_marks_protection.sql) when the student
    // was authorised by DoD for this date — surface that plainly
    // instead of a generic failure message.
    showProfileIncidentError(
      insertError.message?.includes('DOD_AUTHORIZED')
        ? `${studentFullNameForConfirm || 'This student'} was authorised by DoD for ${formatDate(incidentDate)} — marks were not removed.`
        : 'Could not record the incident. Check your connection and try again.'
    );
...
function startEditStudent(id) {
  ...
  studentYearInput.value = student.year_of_birth ?? '';
...
studentForm.addEventListener('submit', async (e) => {
  ...
  const payload = {
    ...
    year_of_birth: studentYearInput.value ? parseInt(studentYearInput.value, 10) : null,
    class_id: classId,
  ...
  if (error) {
    studentFormError.textContent = error.code === '23505'
      ? 'A student with this Student SDMS already exists.'
...
downloadTemplateBtn.addEventListener('click', () => {
  const headers = ['Last Name', 'First Name', 'Student SDMS', 'Gender', 'Year of Birth', 'Class', 'Parent Phone'];
  const example = ['UWIMANA', 'Alice', 430310182491, 'F', 2014, 'P6 A', '0788000000'];
...
function parseYearValue(value) {
  ...
  // Excel sometimes hands back a Date object if a cell was
  // formatted as a date rather than a plain number — pull the
  // year out of it either way.
...
function processImportRows(rawRows) {
  ...
  parsedImportRows = rawRows.map((row, index) => {
    ...
    const studentNumber = getField(row, 'student sdms', 'studentsdms', 'sdms', 'student number', 'studentnumber').toString().trim();
    const genderRaw = getField(row, 'gender').toString().trim().toUpperCase();
    const yearOfBirth = parseYearValue(getField(row, 'year of birth', 'yearofbirth', 'dob', 'date of birth'));
    ...
    if (!firstName || !lastName || !studentNumber) {
      status = 'error';
      note = 'Missing first name, last name, or Student SDMS.';
    } else if (!yearOfBirth || yearOfBirth < 1990 || yearOfBirth > new Date().getFullYear()) {
      status = 'error';
      note = 'Missing or invalid Year of Birth — this is also the login password.';
    } else if (existingNumbers.has(studentNumber.toLowerCase())) {
      status = 'error';
      note = 'Student SDMS already exists in the system.';
    } else if (seenInFile.has(studentNumber.toLowerCase())) {
      status = 'error';
      note = 'Duplicate Student SDMS within this file.';
    } else if (!className) {
      status = 'error';
...
confirmImportBtn.addEventListener('click', async () => {
  ...
  if (error) {
    ...
    importResult.hidden = false;
    importResult.className = 'inline-banner inline-banner--error';
    importResult.textContent = 'Import failed — no students were added. Check for duplicate Student SDMS numbers and try again.';
    return;
  }
...
// Student photos
// ------------------------------------------------------------
// The photo lives in students.photo_url, written only by the
// parent/student portal (supabase/migrations/0008_student_photos.sql).
// Nothing in this admin app writes it — it is read-only here.
...
// Fills one .student-photo element. Initials stay in place as the
// fallback, and are restored if the image fails to load (a photo
// deleted straight out of the bucket, say) so a row never ends up
// showing a broken-image icon.
function setStudentPhoto(imgEl, initialsEl, photoUrl, name) {
  ...
  imgEl.alt = `Photo of ${name ?? 'student'}`;
  imgEl.src = photoUrl;
  imgEl.hidden = false;
}

// Markup version, for rows built as an HTML string.
function studentPhotoMarkup(photoUrl, name, modifier = '') {
  const cls = `student-photo${modifier ? ' ' + modifier : ''}`;
  const img = photoUrl
    ? `<img class="student-photo__img" src="${escapeHtml(photoUrl)}" alt="" onerror="this.hidden = true">`
    : '';
  return `<span class="${cls}">${img}<span class="student-photo__initials">${escapeHtml(getInitials(name))}</span></span>`;
}
