...
function buildWorkbookForClasses(groups) {
  ...
  groups.forEach(({ cls, students }) => {
    const headers = ['#', 'Student SDMS', 'First name', 'Last name', 'Status', 'Marks'];
...
(async function init() {
  ...
  canEdit = profile.role === 'administrator' || profile.role === 'discipline_teacher' || profile.role === 'head_teacher';
  if (!canEdit) {
