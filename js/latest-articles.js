/* =====================================================
   ...
   linking to article/?slug=... and showing its
...
(function () {
  ...
  function renderCards(articles) {
    const cards = document.querySelectorAll("#articleGrid .card, #moreStoriesGrid .card");
    cards.forEach((card, idx) => {
      ...
      card.href = `article/?slug=${encodeURIComponent(a.slug)}`;
      ...
      card.querySelector(".card-meta").textContent = formatAgo(a.published_at || a.created_at);
  ...
  async function init() {
    try {
      ...
      const moreSection = document.getElementById("moreStoriesGrid");
      if (moreSection && data.length > 4) moreSection.style.display = "";
    } catch (err) {
