/* =====================================================
   ...
   into category/'s #catGrid, with a "Load more"
...
(function () {
  ...
  if (!SUPABASE_URL || !SUPABASE_ANON_KEY || !window.supabase) {
    grid.innerHTML = `<div class="empty-state">Couldn't load articles right now. <a href="../">Back to homepage</a></div>`;
    return;
  }
  ...
  function cardHtml(a, idx) {
    const thumbStyle = a.photo_url
      ...
      : "";
    return `
      <a class="card" href="../article/?slug=${encodeURIComponent(a.slug)}" aria-label="Read &quot;${escapeHtml(a.headline)}&quot;">
  ...
  async function loadPage() {
    ...
    try {
      ...
      if (offset === 0 && (!data || data.length === 0)) {
        grid.innerHTML = `<div class="empty-state">No ${cat ? escapeHtml(cat.toLowerCase()) + " " : ""}articles yet — check back soon, or <a href="../">browse the homepage</a>.</div>`;
