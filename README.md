...
js/config.js          ← YOUR SETTINGS: phone number, Telegram, Supabase keys
...
supabase/functions/notify-order/index.ts  ← Edge Function that pings your Telegram on every new booking
...
const TELEGRAM_USERNAME = "sangwastudio_bot"; // your bot's or studio's Telegram @username, no @
```
...
The booking form saves straight to a database and confirms in-page — no Telegram redirect for the customer. **You** get pinged on your own Telegram automatically for every new booking, using a Supabase Edge Function + the official Telegram Bot API.

If Supabase isn't connected yet, the form falls back to the old "open Telegram" flow so you never lose a lead in the meantime.
...
### Getting notified on your own Telegram for every booking AND quick message

This uses the official [Telegram Bot API](https://core.telegram.org/bots/api) — free, instant, and not rate-limited the way the old WhatsApp workaround was. One Edge Function handles both the booking form (`orders` table) and the quick contact form (`messages` table) — it just formats the Telegram text differently depending on which table triggered it.

1. **Get a bot token** from [@BotFather](https://t.me/BotFather) on Telegram (message it `/newbot`, or reuse an existing bot). It replies with a token like `123456789:AAF...`. Keep this secret — anyone with it can send messages as your bot. If a token is ever shared somewhere public (chat log, screenshot, committed file), message @BotFather and use `/revoke` to generate a fresh one before using it.
2. **Get the chat ID that should receive the alerts** (one time):
   - Open Telegram, search for your bot by its @username, and send it any message (e.g. "hi") — this is required once so Telegram allows the bot to message that chat.
   - Then, from any browser, visit (replacing `<TOKEN>` with your real token):
     `https://api.telegram.org/bot<TOKEN>/getUpdates`
   - In the JSON response, find `"chat":{"id":123456789,...}` — that number is your **chat ID**. (If you want the alerts in a group instead, add the bot to the group first, send a message there, then repeat this step — group chat IDs are negative numbers.)
3. **Install the Supabase CLI** if you haven't already, then from this project folder:
   ```bash
   supabase login
   ...
   ```
4. **Set your secrets** (the bot token from @BotFather, and the chat ID from step 2):
   ```bash
   supabase secrets set TELEGRAM_BOT_TOKEN=<your bot token>
   supabase secrets set TELEGRAM_CHAT_ID=<your chat ID>
   ```
5. **Wire up two triggers**: in the Supabase dashboard, go to **Database → Webhooks → Create a new webhook**, and create it twice — once per table:
   - **Webhook 1**
     - Table: `orders`
     - Events: `Insert`
   ...
   - **Webhook 2**
     ...
     - Type: `Supabase Edge Functions`
     - Edge Function: `notify-order`
6. Submit a test booking, then a test quick message, on the live site — you should get a Telegram message for each within a couple of seconds.

The Telegram Bot API is free with no practical rate limit for a solo studio's volume of leads, so there's nothing to outgrow here — unlike the old CallMeBot/WhatsApp setup.
...
- Fully responsive down to mobile, with a slide-in mobile menu, animated stat counters, a filterable portfolio grid (All / Photo / Video / Music), a rotating testimonial carousel, and a floating Telegram button.
...
## 8. Getting articles into Google Search (SEO)

**The problem:** `article/index.html` is one static template shared by every article — the real headline/text only appears after client-side JS fetches it from Supabase. Search engines either don't run that JS at all, or only do so on a slow, unreliable second pass, so every article looked like an identical, empty page to Google — and `sitemap.xml` never listed article URLs in the first place, so Google had almost no way to even discover them.

**The fix** is two Cloudflare Workers + two Supabase Edge Functions, deployed the same way as the existing `share-proxy.js`/`article-og` pair (see their file comments for the exact dashboard steps):

1. **`supabase/functions/article-render`** — given a slug, returns a complete real HTML page: unique `<title>`, description, `<link rel="canonical">`, Open Graph/Twitter tags, `NewsArticle` structured data, and the actual visible headline/dek/byline/photo/body text. No JS required to read it.
2. **`cloudflare-worker/article-dynamic-render.js`** — deployed on the route `gakoromedia.rw/article/*`. Detects known search/social crawlers by User-Agent and routes only those requests to `article-render`; every real visitor is passed straight through to the normal interactive page, completely unchanged. (This is Google's own documented "dynamic rendering" pattern — crawlers and humans end up seeing the same content, just delivered differently.)
3. **`supabase/functions/sitemap-articles`** — generates a live XML sitemap of every published article straight from the `news_articles` table, so it's always current with no manual upkeep.
4. **`cloudflare-worker/sitemap-proxy.js`** — deployed on the route `gakoromedia.rw/sitemap-articles.xml`, proxies to that function.

`sitemap.xml` at the domain root is now a small sitemap **index** pointing at `sitemap-pages.xml` (the static pages, listed by hand — update this if you add a new top-level page) and `sitemap-articles.xml` (the live one above). That index URL — `https://gakoromedia.rw/sitemap.xml` — is what you submit in Google Search Console, not either of the two it points to.

**Deploy checklist:**
- [ ] `supabase functions deploy article-render --no-verify-jwt`
- [ ] `supabase functions deploy sitemap-articles --no-verify-jwt`
- [ ] Add a Cloudflare Worker Route for `gakoromedia.rw/article/*` running `article-dynamic-render.js`
- [ ] Add a Cloudflare Worker Route for `gakoromedia.rw/sitemap-articles.xml` running `sitemap-proxy.js`
- [ ] Submit `https://gakoromedia.rw/sitemap.xml` in Google Search Console → Sitemaps
- [ ] Test: `curl -A "Googlebot" "https://gakoromedia.rw/article/?slug=<a-real-slug>"` should return full article text, not the JS shell
